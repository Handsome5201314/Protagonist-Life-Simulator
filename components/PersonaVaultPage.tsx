...
function createOverlayDraft(personaId: string, overlay?: PersonaOverlay): OverlayDraft {
  return {
    id: overlay?.id,
    personaId,
...
export function PersonaVaultPage({ user, personas, overlays, memoryTraits, worldPacks }: Props) {
  ...
  const [overlayDrafts, setOverlayDrafts] = useState<Record<string, OverlayDraft>>(
    Object.fromEntries(personas.map((persona) => [persona.id, createOverlayDraft(persona.id, overlays.find((item) => item.personaId === persona.id))]))
  );

  const userOwned = useMemo(
    () => personaList.filter((persona) => persona.source !== "legend" && !persona.deletedAt),
    [personaList]
  );
  const linked = user.linkedAiliangbiao?.status === "linked";

  function registerImportedPersona(persona: PersonaSnapshot) {
    setPersonaList((prev) => [persona, ...prev.filter((item) => item.id !== persona.id)]);
    setOverlayDrafts((prev) => ({
      ...prev,
      [persona.id]: createOverlayDraft(persona.id),
    }));
  }

  async function postJson(url: string, body: object, successText: string, onSuccess?: (payload: any) => void) {
    setStatus("");
    try {
  ...
  async function syncAiliangbiao() {
    ...
    try {
      ...
      if (Array.isArray(payload.imported)) {
        setPersonaList((prev) => [
          ...payload.imported,
          ...prev.filter((persona) => !payload.imported.some((item: PersonaSnapshot) => item.id === persona.id)),
        ]);
      }
      startTransition(() => router.refresh());
    } catch (error) {
  ...
  async function submitImportText() {
    if (!importForm.name.trim() || !importForm.rawText.trim()) {
      setStatus("请先填写分身名字和 DNA 文本。");
      return;
    }
    ...
    await postJson(
      ...
      (payload) => {
        if (payload.persona) {
          registerImportedPersona(payload.persona);
  ...
  async function submitImportFile() {
    ...
    try {
      ...
      form.append("ageBand", importForm.ageBand);
      form.append("relation", importForm.relation);
      ...
      setStatus(`文件 ${selectedFile.name} 已导入并生成分身。`);
  ...
  return (
    <main className="mx-auto flex w-full max-w-[1440px] flex-col gap-8 px-4 pb-16 pt-8 md:px-6 lg:px-8">
        ...
        <div className="relative grid gap-6 xl:grid-cols-[1.2fr_0.8fr] xl:items-end">
          <div className="space-y-5">
            ...
            <p className="max-w-4xl text-base leading-8 text-white/72 md:text-lg">
              这里负责统一管理你的数字分身、AIliangbiao 同步档案、数字基因 JSON / TXT 导入，以及用于相亲市场和竞技场的对外人格层。
            </p>
          </div>
      ...
      <section className="grid gap-6 xl:grid-cols-[1fr_1fr]">
        ...
        <div className="rounded-[30px] border border-white/10 bg-white/5 p-6 shadow-2xl backdrop-blur-xl">
          <div>
            <p className="text-sm uppercase tracking-[0.24em] text-white/35">导入 DNA</p>
            <h2 className="mt-2 text-2xl font-black text-white">上传文件或手动铸造分身</h2>
          </div>
          ...
          <div className="mt-5 space-y-5">
            <div className="rounded-[24px] border border-white/10 bg-black/20 p-4">
              <div className="flex flex-wrap items-center gap-3">
                <button
                  ...
                  className="inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/[0.08] px-5 py-3 text-sm font-medium text-white transition hover:bg-white/[0.12]"
                >
                ...
                />
                {selectedFileName ? <span className="text-sm text-white/68">{selectedFileName}</span> : null}
              </div>

              <div className="mt-4 grid gap-3 md:grid-cols-2">
                <div className="rounded-[20px] border border-white/10 bg-white/[0.04] p-4 text-sm leading-7 text-white/72">
                  <div className="flex items-center gap-2 font-semibold text-white">
                    <FileJson className="h-4 w-4 text-cyan-300" />
                    数字基因 JSON
                  </div>
                  <p className="mt-2">支持直接导入 PersonaSnapshot / 数字基因协议 JSON。</p>
                </div>
                <div className="rounded-[20px] border border-white/10 bg-white/[0.04] p-4 text-sm leading-7 text-white/72">
                  <div className="flex items-center gap-2 font-semibold text-white">
                  ...
                  <p className="mt-2">支持导入简历、访谈记录或你整理好的 DNA 文本草稿。</p>
                </div>
              </div>
              ...
              <div className="mt-4 flex flex-wrap items-center gap-3">
                <button
                  ...
                  className="inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/[0.08] px-5 py-3 text-sm font-medium text-white transition hover:bg-white/[0.12] disabled:cursor-not-allowed disabled:opacity-45"
                >
                  <Upload className="h-4 w-4 text-pink-300" />
                  {uploadingFile ? "上传中..." : "上传并生成分身"}
                </button>
                {selectedFile ? <span className="text-sm text-white/55">选中文件后，需要点击这里才会真正上传。</span> : null}
              </div>
            </div>
            ...
            <div className="grid gap-4">
              <input
                ...
                placeholder="分身名字 / Hero Alias"
                className="w-full rounded-[20px] border border-white/10 bg-black/20 px-4 py-3 text-sm text-white outline-none placeholder:text-white/28"
              />
              <div className="grid gap-4 md:grid-cols-2">
                <select
                  value={importForm.ageBand}
                  onChange={(event) => setImportForm((prev) => ({ ...prev, ageBand: event.target.value }))}
                  className="w-full rounded-[20px] border border-white/10 bg-black/20 px-4 py-3 text-sm text-white outline-none"
                >
              ...
              <textarea
                ...
                placeholder="手动粘贴简历、人格描述、访谈记录或你整理好的 DNA 文本..."
                className="min-h-[180px] w-full rounded-[20px] border border-white/10 bg-black/20 px-4 py-3 text-sm leading-7 text-white outline-none placeholder:text-white/28"
              />
              <button
                type="button"
                disabled={isPending}
                onClick={() => void submitImportText()}
                className="inline-flex w-fit items-center gap-2 rounded-full border border-white/10 bg-white/[0.08] px-5 py-3 text-sm font-medium text-white transition hover:bg-white/[0.12]"
              >
                <Upload className="h-4 w-4 text-pink-300" />
                从文本铸造分身 DNA
              </button>
      ...
      {status ? (
        <div className="rounded-[24px] border border-pink-300/20 bg-pink-400/10 px-5 py-4 text-sm text-pink-50">
          {status}
        </div>
      ) : null}
      ...
      <section className="space-y-4">
                  ...
                  <div className="space-y-4">
                    <div>
                      ...
                      <textarea
                        ...
                        onChange={(event) => setOverlayDrafts((prev) => ({ ...prev, [persona.id]: { ...prev[persona.id], resumeSummary: event.target.value } }))}
                        className="min-h-[120px] w-full rounded-[20px] border border-white/10 bg-black/20 px-4 py-3 text-sm leading-7 text-white outline-none placeholder:text-white/28"
                        placeholder="写给市场和剧本系统看的履历摘要..."
                      />
                    </div>
                    <div>
                      ...
                      <textarea
                        ...
                        onChange={(event) => setOverlayDrafts((prev) => ({ ...prev, [persona.id]: { ...prev[persona.id], publicBio: event.target.value } }))}
                        className="min-h-[120px] w-full rounded-[20px] border border-white/10 bg-black/20 px-4 py-3 text-sm leading-7 text-white outline-none placeholder:text-white/28"
                        placeholder="写给相亲市场和围观大厅看的对外版本..."
                      />
                    </div>
                    <button
                      type="button"
                      disabled={isPending}
                      onClick={() => void saveOverlay(persona.id)}
                      className="inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/[0.08] px-5 py-3 text-sm font-medium text-white transition hover:bg-white/[0.12]"
                    >
        ...
        {!userOwned.length ? (
          <div className="rounded-[28px] border border-dashed border-white/12 bg-white/5 p-10 text-center shadow-2xl backdrop-blur-xl">
            ...
            <p className="mt-3 text-sm leading-7 text-white/55">先同步 AIliangbiao，或者导入一个本地 JSON/TXT 文件，系统就会为你写入第一位分身。</p>
          </div>
        ) : null}
      </section>
    </main>
