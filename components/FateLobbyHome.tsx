...
import type { FateCategory, FateRoomCard } from "@/lib/fate-arena";
...
function categoryTone(category: FateCategory) {
  if (category === "survival") {
    return {
      ...
      glow: "from-cyan-500/30 via-sky-500/12 to-transparent",
    };
  }
  ...
  if (category === "business") {
    return {
      ...
      glow: "from-amber-500/30 via-orange-500/12 to-transparent",
    };
  }

  return {
    badge: "border-purple-300/30 bg-purple-400/10 text-purple-100",
    glow: "from-purple-500/30 via-violet-500/15 to-transparent",
  };
}

function seatTone(hue: FateRoomCard["roster"][number]["hue"]) {
...
export function FateLobbyHome({ user, rooms }: Props) {
    ...
    <main className="mx-auto flex w-full max-w-[1440px] flex-col gap-8 px-4 pb-16 pt-8 md:px-6 lg:px-8">
        ...
        <div className="relative grid gap-6 xl:grid-cols-[1.2fr_0.8fr] xl:items-center">
          <div className="space-y-5">
            ...
            <p className="max-w-4xl text-base leading-8 text-white/72 md:text-lg">
              命运大厅现在专门承载废土、生存、商战、秘仪与悬疑类剧本。图灵相亲局已经被独立拆分到“相亲市场”，两条入口的内容和视觉都将明确区分。
            </p>
            ...
            {featuredRoom ? (
              <div className="grid gap-4 rounded-[28px] border border-white/10 bg-black/20 p-5 md:grid-cols-[1fr_auto] md:items-end">
                ...
                <Link
                  href={featuredRoom.status === "recruiting" ? featuredRoom.prepHref : featuredRoom.roomHref}
                  className="inline-flex items-center justify-center gap-2 rounded-full bg-gradient-to-r from-cyan-500 to-purple-500 px-6 py-3 text-sm font-semibold text-white shadow-[0_0_26px_rgba(34,211,238,0.22)] transition hover:translate-y-[-1px]"
                >
          ...
          <div className="rounded-[30px] border border-white/10 bg-black/20 p-6 shadow-2xl">
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
                ...
                <strong className="mt-2 block text-2xl font-black text-white">
                  {rooms.reduce((sum, room) => sum + Math.max(room.maxPlayers - room.players, 0), 0)}
                </strong>
              </div>
            </div>
            ...
            <div className="mt-4 rounded-[26px] border border-white/10 bg-white/5 p-5">
              <p className="text-sm leading-7 text-white/72">
                想进入图灵相亲、1v1 邂逅和心动爆灯类内容，请直接前往顶部导航里的“相亲市场”。
              </p>
            </div>

            <Link
              href={isLinked ? "/dating" : "/api/auth/agentpit/login"}
              className={`mt-4 inline-flex w-full items-center justify-center gap-2 rounded-full px-5 py-4 text-sm font-semibold transition ${
                isLinked
                  ? "border border-white/15 bg-white/[0.08] text-white hover:bg-white/[0.12]"
                  : "bg-gradient-to-r from-purple-500 to-fuchsia-500 text-white shadow-[0_0_24px_rgba(192,132,252,0.34)] hover:translate-y-[-1px]"
              }`}
            >
              ...
              return (
                <button
                  ...
                  className={`inline-flex items-center gap-2 rounded-xl border px-4 py-2.5 text-sm font-medium transition ${
                    active
                      ? "border-cyan-300/35 bg-cyan-400/10 text-cyan-100 shadow-[0_0_18px_rgba(34,211,238,0.16)]"
      ...
      <section className="space-y-4">
        <div className="flex items-center justify-between gap-4">
          <div>
            <p className="text-sm uppercase tracking-[0.24em] text-white/35">Destiny Grid</p>
        ...
        <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-3">
          {filteredRooms.map((room) => {
            ...
            return (
              <article key={room.id} className="group relative overflow-hidden rounded-[30px] border border-white/10 bg-white/5 p-6 shadow-2xl backdrop-blur-xl transition hover:border-white/20 hover:bg-white/[0.075]">
                ...
                <div className="relative space-y-6">
                  <div className="flex items-start justify-between gap-4">
                    <div className="space-y-3">
                      <div className="flex flex-wrap gap-2">
                        {room.typeTags.map((tag) => (
                          <span key={tag} className={`inline-flex items-center rounded-full border px-3 py-1 text-[0.7rem] font-semibold uppercase tracking-[0.18em] ${tone.badge}`}>
                            {tag}
                          </span>
                        ))}
                      </div>
                    ...
                    <span className={`rounded-full border px-3 py-1 text-xs font-semibold ${room.status === "running" ? "border-cyan-300/25 bg-cyan-400/10 text-cyan-100" : room.status === "replay" ? "border-amber-300/25 bg-amber-400/10 text-amber-100" : "border-white/10 bg-white/5 text-white/68"}`}>
                      {room.statusLabel}
                    </span>
                  </div>
                  ...
                  <div className="flex flex-wrap gap-3">
                    {room.roster.slice(0, Math.max(room.players, 3)).map((seat) => (
                      <div key={seat.id} className="inline-flex items-center gap-3 rounded-full border border-white/10 bg-black/20 px-3 py-2">
                  ...
                  <div className="flex items-center justify-between gap-4 border-t border-white/10 pt-4">
                    <div className="flex gap-4">
                      <div className="flex flex-col">
                        ...
                        <span className="text-xs font-bold text-white/80 inline-flex items-center gap-1"><Users className="h-3 w-3" />{room.players}/{room.maxPlayers}</span>
                      </div>
                      <div className="flex flex-col">
                        <span className="text-[10px] text-white/40">围观</span>
                        <span className="text-xs font-bold text-white/80 inline-flex items-center gap-1"><Eye className="h-3 w-3" />{room.spectators.toLocaleString()}</span>
                      </div>
                      <div className="flex flex-col">
                        <span className="text-[10px] text-white/40">奖金池</span>
                        <span className="text-xs font-bold text-amber-300 inline-flex items-center gap-1"><Coins className="h-3 w-3" />{room.prizePool.toLocaleString()}</span>
                      </div>
                    </div>

                    <Link
                      href={room.status === "recruiting" ? room.prepHref : room.roomHref}
                      className={`inline-flex items-center gap-2 rounded-xl px-4 py-2.5 text-sm font-bold text-white transition ${
                        room.status === "recruiting"
                          ? "bg-gradient-to-r from-cyan-500 to-purple-500 shadow-[0_0_20px_rgba(34,211,238,0.18)] hover:translate-y-[-1px]"
                          : "border border-white/20 bg-white/[0.08] hover:bg-white/[0.12]"
                      }`}
                    >
                      {room.status === "recruiting" ? "注入分身" : "立即围观"}
                    </Link>
                  </div>
                </div>
              </article>
