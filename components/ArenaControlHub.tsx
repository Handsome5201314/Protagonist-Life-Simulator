...
export function ArenaControlHub({ locale, user, personas, worldPacks, matches, participants, tickets }: Props) {
  ...
  const rooms = useMemo(
    () => buildFateLobbyRooms({ worldPacks, matches, participants, personas }),
  ...
  async function createArenaMatch() {
    ...
    try {
      ...
      if (!response.ok) throw new Error(payload.error || t("Failed to create arena room", "创建竞技房失败"));
      setStatus(t("Arena room created.", "新的竞技房已创建。"));
      startTransition(() => router.push(`/arena/prep/${payload.match.id}`));
    } catch (error) {
      setStatus(error instanceof Error ? error.message : t("Failed to create arena room", "创建竞技房失败"));
    }
  }
    ...
    <main className="mx-auto flex w-full max-w-[1440px] flex-col gap-8 px-4 pb-16 pt-8 md:px-6 lg:px-8">
        ...
        <div className="relative grid gap-6 xl:grid-cols-[1.15fr_0.85fr] xl:items-end">
          <div className="space-y-5">
            <span className="inline-flex items-center gap-2 rounded-full border border-pink-300/25 bg-pink-400/10 px-3 py-1 text-xs font-semibold uppercase tracking-[0.2em] text-pink-100">
              ...
              {t("Arena Command", "竞技场控制台")}
            </span>
            <div>
              <p className="text-sm uppercase tracking-[0.26em] text-white/35">{t("Battle Control", "战局放映厅")}</p>
              <h1 className="mt-3 text-3xl font-black text-white md:text-5xl">{t("Create and manage live arena rooms", "创建并管理实时竞技房")}</h1>
            </div>
            <p className="max-w-4xl text-base leading-8 text-white/72 md:text-lg">
              {t(
                "Use this console to pick a world, seat clones, and jump directly into prep or the live room.",
                "你可以在这里选择世界、安排分身上桌，并直接跳转到准备室或房内互动界面。"
              )}
            </p>
          </div>
          <div className="grid gap-3 sm:grid-cols-3 xl:grid-cols-1 2xl:grid-cols-3">
            <div className="rounded-2xl border border-white/10 bg-black/20 p-4">
              <p className="text-xs uppercase tracking-[0.18em] text-white/35">{t("Room Count", "房间总数")}</p>
              <strong className="mt-2 block text-2xl font-black text-white">{rooms.length}</strong>
            </div>
            <div className="rounded-2xl border border-white/10 bg-black/20 p-4">
              <p className="text-xs uppercase tracking-[0.18em] text-white/35">{t("Renown", "围观声望")}</p>
            ...
            <div className="rounded-2xl border border-white/10 bg-black/20 p-4">
              <p className="text-xs uppercase tracking-[0.18em] text-white/35">{t("Support Tickets", "支援记录")}</p>
              <strong className="mt-2 inline-flex items-center gap-2 text-lg font-black text-white">
      ...
      <section className="grid gap-6 xl:grid-cols-[1.1fr_0.9fr]">
        <div className="rounded-[30px] border border-white/10 bg-white/5 p-6 shadow-2xl backdrop-blur-xl">
          <p className="text-sm uppercase tracking-[0.24em] text-white/35">{t("Create Table", "创建牌桌")}</p>
          <h2 className="mt-2 text-2xl font-black text-white">{t("Open a new arena room", "开启新的竞技房")}</h2>

          <div className="mt-5 grid gap-5">
            <label className="block">
              <span className="mb-2 block text-sm font-medium text-white/72">{t("World Pack", "剧本世界包")}</span>
            ...
            <div>
              <span className="mb-3 block text-sm font-medium text-white/72">{t("Seat Clones", "上桌分身")}</span>
                  ...
                  return (
                    <button
                      ...
                      className={`rounded-[22px] border p-4 text-left transition ${
                        active ? "border-pink-300/30 bg-pink-400/10" : "border-white/10 bg-black/20 hover:bg-white/[0.07]"
                      }`}
                    >
            ...
            <button
              type="button"
              disabled={isPending || !selectedPersonaIds.length || !selectedWorld}
              onClick={() => void createArenaMatch()}
              className="inline-flex w-fit items-center gap-2 rounded-full bg-gradient-to-r from-pink-500 to-purple-500 px-5 py-3 text-sm font-semibold text-white shadow-[0_0_24px_rgba(232,121,249,0.28)] transition hover:translate-y-[-1px] disabled:cursor-not-allowed disabled:opacity-45"
            >
              <PlusCircle className="h-4 w-4" />
              {t("Enter Prep Room", "进入准备室")}
            </button>
          </div>
        </div>

        <div className="rounded-[30px] border border-white/10 bg-white/5 p-6 shadow-2xl backdrop-blur-xl">
          <p className="text-sm uppercase tracking-[0.24em] text-white/35">{t("Live Rooms", "当前牌桌")}</p>
          <h2 className="mt-2 text-2xl font-black text-white">{t("Rooms on air", "正在运行的房间")}</h2>
            ...
            {rooms.slice(0, 5).map((room: FateRoomCard) => (
              <article key={room.id} className="rounded-[24px] border border-white/10 bg-black/20 p-4">
                <div className="flex items-start justify-between gap-4">
                  ...
                  <span className={`rounded-full border px-3 py-1 text-xs ${room.status === "recruiting" || room.isPreviewRoom ? "border-white/10 bg-white/5 text-white/68" : "border-pink-300/25 bg-pink-400/10 text-pink-100"}`}>
                    {room.statusLabel}
                  </span>
                </div>
                <div className="mt-4 grid grid-cols-3 gap-3 text-sm">
                  ...
                  <div className="rounded-2xl border border-white/10 bg-white/[0.04] p-3 text-white/72">
                    <Eye className="mb-2 h-4 w-4 text-white/45" />
                    {room.spectators}
                  </div>
                  <div className="rounded-2xl border border-white/10 bg-white/[0.04] p-3 text-white/72">
                    <Flame className="mb-2 h-4 w-4 text-white/45" />
                    {room.prizePool}
                  </div>
                </div>
                <Link href={room.ctaHref} className="mt-4 inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/[0.08] px-4 py-2.5 text-sm font-medium text-white transition hover:bg-white/[0.12]">
                  {room.ctaLabel}
      ...
      <section className="rounded-[30px] border border-white/10 bg-white/5 p-6 shadow-2xl backdrop-blur-xl">
        <div className="flex items-center gap-3">
          ...
          <h2 className="text-2xl font-black text-white">{t("Operator Notes", "运营说明")}</h2>
        </div>
        <div className="mt-5 grid gap-4 md:grid-cols-3">
          <div className="rounded-[24px] border border-white/10 bg-black/20 p-4 text-sm leading-7 text-white/68">
            {t("Renown is used for support and loadout, not for direct outcome buying.", "声望用于围观支援和技能挂载，不参与直接买胜负。")}
          </div>
          <div className="rounded-[24px] border border-white/10 bg-black/20 p-4 text-sm leading-7 text-white/68">
            {t("Seat clones in prep first, then move into live interaction.", "先在准备室编排席位，再进入房内互动。")}
          </div>
          <div className="rounded-[24px] border border-white/10 bg-black/20 p-4 text-sm leading-7 text-white/68">
            {t("All arena rooms inherit the same house style so the world still feels coherent.", "所有竞技房都继承同一套视觉语言，保证世界观连贯。")}
          </div>
        </div>
      </section>
    </main>
