...
import type { ArenaProxyMode, PersonaOverlay, PersonaSnapshot } from "@/lib/types";
...
const proxyOptions: Array<{
...
}> = [
  {
    id: "self",
    title: "亲自参与推演",
    summary: "你在主战场手动驱动行动与技能，适合细调每一步试探。",
    icon: Keyboard,
  },
  {
    id: "ai",
    title: "AI 代理托管",
    summary: "系统将沿着 briefing 和人格画像自动推进分身，让你以观战者视角看完整场。",
    icon: Bot,
  },
];
...
export function ArenaPrepRoom({ prepView, personas = [], overlays = [] }: Props) {
  ...
  const [proxyMode, setProxyMode] = useState<ArenaProxyMode>(prepView.proxyMode);
  const [briefing, setBriefing] = useState(prepView.briefing);
  ...
  useEffect(() => {
    ...
    setProxyMode(prepView.proxyMode);
    setBriefing(prepView.briefing);
  ...
  async function persist(
    nextMode: FateModeId,
    nextProxyMode: ArenaProxyMode,
    nextBriefing: string,
    nextActive: FateSeat[],
    nextReserve: FateSeat[],
    successText: string
  ) {
  ...
  async function handleModeChange(mode: FateModeId) {
    ...
    await persist(mode, proxyMode, briefing, activeSeats, reserveSeats, `已切换到${mode === "rapid" ? "极速推演" : "沉浸互动"}模式。`);
  }

  async function handleProxyChange(nextProxyMode: ArenaProxyMode) {
    if (nextProxyMode === proxyMode) return;
    await persist(
      activeMode,
      nextProxyMode,
      briefing,
      activeSeats,
      reserveSeats,
      nextProxyMode === "ai" ? "已切换到 AI 代理托管。" : "已切换到亲自参与推演。"
    );
  }

  async function handleBriefingSave() {
    const nextBriefing = briefing.trim();
    if (!nextBriefing) {
      setStatusText("请先补充 briefing，再保存。");
      return;
    }
    await persist(activeMode, proxyMode, nextBriefing, activeSeats, reserveSeats, "对局 briefing 已保存。");
  }
  ...
  async function handleShuffle() {
    ...
    await persist(activeMode, proxyMode, briefing, nextActive, finalReserve, `已将 ${incoming.name} 调入当前席位。`);
  }
  ...
  async function handleInvite() {
    ...
    const incoming = nextReserve.shift();
    if (!incoming) return;
    await persist(activeMode, proxyMode, briefing, [...activeSeats, incoming], nextReserve, `${incoming.name} 已完成入座。`);
  }
      ...
      <main className="mx-auto flex w-full max-w-[1320px] flex-col gap-8 px-4 pb-16 pt-8 md:px-6 lg:px-8">
              ...
              <div className="mt-5 grid gap-4">
                <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
                  ...
                  <strong className="mt-2 block text-2xl font-black text-white">
                    {activeSeats.length}/{prepView.maxPlayers}
                  </strong>
                </div>
                <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
                  <p className="text-xs uppercase tracking-[0.18em] text-white/35">推演模式</p>
                  <strong className="mt-2 block text-lg font-bold text-white">
                    {activeMode === "rapid" ? "极速推演" : "沉浸互动"}
                  </strong>
                </div>
                <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
                  <p className="text-xs uppercase tracking-[0.18em] text-white/35">控制方式</p>
                  <strong className="mt-2 block text-lg font-bold text-white">
                    {proxyMode === "ai" ? "AI 代理托管" : "亲自参与推演"}
                  </strong>
                </div>
                <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
        ...
        <section className="rounded-[34px] border border-white/10 bg-white/5 p-6 shadow-2xl backdrop-blur-xl md:p-8">
          <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
            <div>
              <p className="text-sm uppercase tracking-[0.28em] text-white/40">对局说明</p>
              <h2 className="mt-2 text-2xl font-black text-white md:text-3xl">房间 briefing</h2>
              <p className="mt-3 max-w-3xl text-sm leading-7 text-white/66">
                这里会成为裁判引擎、Narrator Agent 和后续托管逻辑共享的准备态描述。建议写清舞台背景、第一轮试探方向和对局风险。
              </p>
            </div>
            <div className="flex flex-wrap gap-3">
              <button
                type="button"
                onClick={() => setBriefing(prepView.description)}
              ...
              <button
                type="button"
                disabled={saving || !prepView.canPersist}
                onClick={() => void handleBriefingSave()}
                className="inline-flex items-center gap-2 rounded-full border border-pink-300/30 bg-pink-400/10 px-4 py-2.5 text-sm text-pink-100 transition hover:bg-pink-400/15 disabled:cursor-not-allowed disabled:opacity-45"
              >
          ...
          <textarea
            ...
            className="mt-6 min-h-[180px] w-full rounded-[28px] border border-white/10 bg-black/20 px-5 py-4 text-sm leading-8 text-white outline-none placeholder:text-white/28"
            placeholder="写下这一局的房间设定、冲突焦点、预期时长与第一轮打法。"
          />
        </section>
        ...
        <section className="grid gap-6 lg:grid-cols-2">
          {proxyOptions.map((option) => {
            const Icon = option.icon;
            const active = option.id === proxyMode;
            return (
              <button
                key={option.id}
                type="button"
                disabled={saving}
                onClick={() => void handleProxyChange(option.id)}
                className={`rounded-[30px] border p-6 text-left shadow-2xl backdrop-blur-xl transition ${
                  active
                    ? "border-cyan-300/35 bg-cyan-400/10 shadow-[0_0_30px_rgba(34,211,238,0.14)]"
                    : "border-white/10 bg-white/5 hover:border-white/20 hover:bg-white/[0.075]"
                }`}
              >
          ...
          <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            ...
            <div className="flex flex-wrap gap-3">
              <button
                ...
                className="inline-flex items-center gap-2 rounded-full border border-white/10 bg-black/20 px-4 py-2.5 text-sm text-white/76 transition hover:bg-white/10 hover:text-white disabled:cursor-not-allowed disabled:opacity-45"
              >
            ...
            {activeSeats.map((seat, index) => (
              ...
              >
                ...
                <div className="relative space-y-4">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {seat.tags.map((tag) => (
                      <span key={tag} className="inline-flex items-center rounded-full border border-white/10 bg-white/5 px-3 py-1 text-xs text-white/70">
                        {tag}
                      </span>
                    ))}
                    {seat.isUserOwned ? (
            ...
            <div className="mt-6 rounded-[28px] border border-white/10 bg-black/20 p-5">
              ...
              <div className="flex flex-wrap gap-3">
                {reserveSeats.map((seat) => (
                  ...
                  >
                    <div className={`grid h-8 w-8 place-items-center rounded-full bg-gradient-to-br ${seatTone(seat.hue)} text-xs font-bold text-white`}>
                      {seat.name.slice(0, 1)}
                    </div>
                    <div>
        ...
        <section className="relative overflow-hidden rounded-[34px] border border-white/10 bg-white/5 p-6 shadow-2xl backdrop-blur-xl md:p-8">
          ...
          <div className="relative mx-auto flex max-w-3xl flex-col items-center gap-5 text-center">
            <div className="space-y-3">
              <p className="text-sm uppercase tracking-[0.28em] text-white/35">启动序列</p>
              ...
              <p className="text-sm leading-7 text-white/60 md:text-base">
                当前模式为 {activeMode === "rapid" ? "极速推演" : "沉浸互动"}，控制方式为
                {proxyMode === "ai" ? " AI 代理托管" : " 亲自参与推演"}。
                系统会继承你现在的入座阵容和 briefing 进入主战场。
              </p>
            </div>

            {prepView.canPersist ? (
              <Link
                href={prepView.roomHref}
                className="inline-flex min-w-[18rem] items-center justify-center gap-3 rounded-full bg-gradient-to-r from-pink-500 via-fuchsia-500 to-purple-500 px-8 py-5 text-lg font-bold text-white shadow-[0_0_36px_rgba(232,121,249,0.38)] transition hover:translate-y-[-1px] hover:shadow-[0_0_48px_rgba(232,121,249,0.5)]"
              >
                <Rocket className="h-5 w-5" />
                {proxyMode === "ai" ? "开启 AI 托管推演" : "开启命运推演"}
              ...
              <button
                type="button"
                disabled
                className="inline-flex min-w-[18rem] items-center justify-center gap-3 rounded-full bg-white/10 px-8 py-5 text-lg font-bold text-white/50"
              >
                <Rocket className="h-5 w-5" />
                预览房间暂不可启动
              </button>
