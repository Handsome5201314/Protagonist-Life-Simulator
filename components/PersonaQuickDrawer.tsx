...
import { Dna, Fingerprint, ShieldCheck, Sparkles, X } from "lucide-react";
...
const traitMeta: Record<string, { label: string; color: string }> = {
  charm: { label: "魅力", color: "from-pink-400 to-rose-500" },
  resilience: { label: "韧性", color: "from-amber-400 to-orange-500" },
  focus: { label: "专注", color: "from-cyan-400 to-blue-500" },
  empathy: { label: "共情", color: "from-violet-400 to-purple-500" },
  strategy: { label: "谋略", color: "from-indigo-400 to-violet-500" },
  chaos: { label: "变数", color: "from-fuchsia-400 to-pink-500" },
  courage: { label: "勇气", color: "from-emerald-400 to-teal-500" },
};
...
export function PersonaQuickDrawer({ open, persona, overlay, memories = [], onClose }: Props) {
  ...
  return (
    <>
      ...
      <aside
        className={`fixed right-0 top-0 z-[90] flex h-full w-full max-w-[440px] flex-col overflow-y-auto border-l border-white/10 bg-gradient-to-b from-[#0f0c29]/98 via-[#1a1540]/98 to-[#24243e]/98 shadow-2xl backdrop-blur-xl transition-transform duration-300 ease-out ${
        ...
        <div className="flex flex-col gap-6 p-6">
          <div className="flex items-center gap-5">
            ...
            <div className="min-w-0">
              ...
              <div className="mt-1 flex flex-wrap gap-2">
                ...
                >
                  {persona.adultOnlyEligible ? "公开竞技可用" : "受保护"}
                </span>
              </div>
            </div>
          </div>
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <div className="rounded-[26px] border border-white/10 bg-black/20 p-4">
              ...
              <p className="text-sm leading-7 text-white/72">{persona.interests.join(" / ") || "暂无公开兴趣"}</p>
            </div>
            <div className="rounded-[26px] border border-white/10 bg-black/20 p-4">
              <p className="mb-2 text-[0.68rem] font-semibold uppercase tracking-[0.16em] text-white/45">顾虑</p>
              <p className="text-sm leading-7 text-white/72">{persona.fears.join(" / ") || "暂无公开顾虑"}</p>
            </div>
          </div>
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <div className="rounded-[26px] border border-white/10 bg-black/20 p-4">
              ...
              <p className="text-sm leading-7 text-white/72">{persona.communicationStyle || "未设置"}</p>
            </div>
            <div className="rounded-[26px] border border-white/10 bg-black/20 p-4">
              <p className="mb-2 text-[0.68rem] font-semibold uppercase tracking-[0.16em] text-white/45">职业倾向</p>
              <p className="text-sm leading-7 text-white/72">{persona.careerTilt || "未设置"}</p>
            </div>
          </div>
          ...
          {overlay ? (
            <div className="rounded-[26px] border border-white/10 bg-black/20 p-5">
              ...
              {overlay.resumeSummary ? (
                <div className="mb-3">
                  <p className="mb-1 text-[0.68rem] font-semibold uppercase tracking-[0.16em] text-white/45">履历摘要</p>
                  <p className="text-sm leading-7 text-white/72">{overlay.resumeSummary}</p>
                </div>
              ) : null}
              {overlay.publicBio ? (
                <div>
                  <p className="mb-1 text-[0.68rem] font-semibold uppercase tracking-[0.16em] text-white/45">公开人设</p>
