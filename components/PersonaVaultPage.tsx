...
export function PersonaVaultPage({ user, personas, overlays, memoryTraits, worldPacks }: Props) {
  ...
  const [importForm, setImportForm] = useState({ name: "", rawText: "", ageBand: "adult", relation: "SELF" });
      ...
      <section className="relative overflow-hidden rounded-[34px] border border-white/10 bg-white/5 p-6 shadow-2xl backdrop-blur-xl md:p-8">
        ...
        <div className="relative grid gap-6 xl:grid-cols-[1.2fr_0.8fr] xl:items-end">
          <div className="space-y-5">
            <span className="inline-flex items-center gap-2 rounded-full border border-pink-300/25 bg-pink-400/10 px-3 py-1 text-xs font-semibold uppercase tracking-[0.2em] text-pink-100"><Dna className="h-3.5 w-3.5" />Persona DNA Vault</span>
            <div>
            ...
            <p className="max-w-4xl text-base leading-8 text-white/72 md:text-lg">这里负责统一管理你的数字分身、AIliangbiao 同步档案、上传式 DNA 草稿，以及用于相亲市场和竞技场的对外人格层。</p>
          </div>

          <div className="grid gap-3 sm:grid-cols-3 xl:grid-cols-1 2xl:grid-cols-3">
            <div className="rounded-2xl border border-white/10 bg-black/20 p-4"><p className="text-xs uppercase tracking-[0.18em] text-white/35">已收录分身</p><strong className="mt-2 block text-2xl font-black text-white">{userOwned.length}</strong></div>
            <div className="rounded-2xl border border-white/10 bg-black/20 p-4"><p className="text-xs uppercase tracking-[0.18em] text-white/35">AIliangbiao</p><strong className="mt-2 block text-lg font-black text-white">{linked ? "已连接" : "未连接"}</strong></div>
            <div className="rounded-2xl border border-white/10 bg-black/20 p-4"><p className="text-xs uppercase tracking-[0.18em] text-white/35">剧本挂接</p><strong className="mt-2 block text-2xl font-black text-white">{worldPacks.length}</strong></div>
          </div>
        </div>
      </section>
      ...
      <section className="grid gap-6 xl:grid-cols-[1fr_1fr]">
        <div className="rounded-[30px] border border-white/10 bg-white/5 p-6 shadow-2xl backdrop-blur-xl">
          <div className="flex items-start justify-between gap-4">
            ...
            <span className={`rounded-full border px-3 py-1 text-xs ${linked ? "border-emerald-300/25 bg-emerald-400/10 text-emerald-100" : "border-white/10 bg-white/5 text-white/65"}`}>{linked ? "已连接" : "未连接"}</span>
          </div>
          <p className="mt-4 text-sm leading-7 text-white/68">一键拉取 AIliangbiao 原型分身 DNA，并自动去重写入本地分身库。同步后的分身会保留来源标签，可继续补充对外人格层。</p>
          <div className="mt-5 flex flex-wrap gap-3">
            <button type="button" disabled={isPending} onClick={() => void syncAiliangbiao()} className="inline-flex items-center gap-2 rounded-full bg-gradient-to-r from-pink-500 to-purple-500 px-5 py-3 text-sm font-semibold text-white shadow-[0_0_24px_rgba(232,121,249,0.28)] transition hover:translate-y-[-1px]"><DatabaseZap className="h-4 w-4" />同步 AIliangbiao 分身 DNA</button>
            <a href="/api/auth/agentpit/login" className="inline-flex items-center gap-2 rounded-full border border-white/10 bg-black/20 px-5 py-3 text-sm font-medium text-white/80 transition hover:bg-white/[0.08]"><Sparkles className="h-4 w-4 text-cyan-300" />AgentPit 授权登录</a>
          </div>
        </div>
        ...
        <div className="rounded-[30px] border border-white/10 bg-white/5 p-6 shadow-2xl backdrop-blur-xl">
          ...
          <div className="mt-5 grid gap-4">
            ...
            <button type="button" disabled={isPending} onClick={() => void submitImport()} className="inline-flex w-fit items-center gap-2 rounded-full border border-white/10 bg-white/[0.08] px-5 py-3 text-sm font-medium text-white transition hover:bg-white/[0.12]"><Upload className="h-4 w-4 text-pink-300" />上传并铸造分身 DNA</button>
          </div>
        </div>
      </section>
      ...
      <section className="space-y-4">
              ...
              <article key={persona.id} className="rounded-[30px] border border-white/10 bg-white/5 p-6 shadow-2xl backdrop-blur-xl">
                <div className="grid gap-6 xl:grid-cols-[1fr_1fr]">
                  <div className="space-y-4">
                    <div className="flex items-start justify-between gap-4">
                      <div className="flex items-center gap-4">
                        <div className="grid h-14 w-14 place-items-center rounded-2xl bg-gradient-to-br from-pink-400 via-purple-400 to-cyan-400 text-lg font-black text-white">{persona.name.slice(0, 1)}</div>
                        <div>
                      ...
                      <span className={`rounded-full border px-3 py-1 text-xs ${persona.adultOnlyEligible ? "border-emerald-300/25 bg-emerald-400/10 text-emerald-100" : "border-amber-300/25 bg-amber-400/10 text-amber-100"}`}>{persona.adultOnlyEligible ? "可公开出战" : "仅私密使用"}</span>
                    </div>

                    <div className="rounded-[24px] border border-white/10 bg-black/20 p-4 text-sm leading-7 text-white/72">
                      <div className="flex items-center gap-2 text-white/55"><Fingerprint className="h-4 w-4 text-pink-300" />Locked DNA Hash</div>
                      <p className="mt-2 break-all text-xs text-white/45">{persona.lockedHash}</p>
                    </div>

                    <div className="flex flex-wrap gap-2">{persona.publicTraitTags.map((tag) => <span key={tag} className="rounded-full border border-white/10 bg-black/20 px-3 py-1 text-xs text-white/72">{tag}</span>)}</div>
                    <div className="grid gap-3 sm:grid-cols-2">
                      <div className="rounded-[22px] border border-white/10 bg-black/20 p-4"><p className="text-xs uppercase tracking-[0.18em] text-white/35">兴趣</p><p className="mt-2 text-sm leading-7 text-white/72">{persona.interests.join(" / ") || "暂无"}</p></div>
                      <div className="rounded-[22px] border border-white/10 bg-black/20 p-4"><p className="text-xs uppercase tracking-[0.18em] text-white/35">顾虑</p><p className="mt-2 text-sm leading-7 text-white/72">{persona.fears.join(" / ") || "暂无"}</p></div>
                    </div>

                    {memories.length ? <div className="rounded-[22px] border border-white/10 bg-black/20 p-4"><div className="flex items-center gap-2 text-sm font-semibold text-white"><ShieldCheck className="h-4 w-4 text-cyan-300" />记忆碎片</div><div className="mt-3 flex flex-wrap gap-2">{memories.map((memory) => <span key={memory.id} className="rounded-full border border-white/10 bg-white/[0.06] px-3 py-1 text-xs text-white/68">{memory.name}</span>)}</div></div> : null}
                  </div>
                  ...
                  <div className="space-y-4">
                    ...
                    <button type="button" disabled={isPending} onClick={() => void saveOverlay(persona.id)} className="inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/[0.08] px-5 py-3 text-sm font-medium text-white transition hover:bg-white/[0.12]"><LockKeyhole className="h-4 w-4 text-pink-300" />保存外显人格层</button>
                  </div>
                </div>
              </article>
            );
        ...
        {!userOwned.length ? (
          <div className="rounded-[28px] border border-dashed border-white/12 bg-white/5 p-10 text-center shadow-2xl backdrop-blur-xl">
            <div className="mx-auto grid h-16 w-16 place-items-center rounded-3xl border border-white/10 bg-black/20 text-white/55"><UserRound className="h-6 w-6" /></div>
