  ...
  return (
    <main className="mx-auto flex w-full max-w-[1440px] flex-col gap-8 px-4 pb-16 pt-8 md:px-6 lg:px-8">
      <section className="relative overflow-hidden rounded-[34px] border border-white/10 bg-white/5 p-6 shadow-2xl backdrop-blur-xl md:p-8">
        ...
        <div className="relative grid gap-6 xl:grid-cols-[1.15fr_0.85fr] xl:items-end">
          <div className="space-y-5">
            <span className="inline-flex items-center gap-2 rounded-full border border-pink-300/25 bg-pink-400/10 px-3 py-1 text-xs font-semibold uppercase tracking-[0.2em] text-pink-100"><Swords className="h-3.5 w-3.5" />Arena Command</span>
            <div>
            ...
            <p className="max-w-4xl text-base leading-8 text-white/72 md:text-lg">在这里可以快速开局、选择世界包、编排分身上桌，并跳转到准备室或互动室。它是大厅之外的战局运营入口。</p>
          </div>
          <div className="grid gap-3 sm:grid-cols-3 xl:grid-cols-1 2xl:grid-cols-3">
            <div className="rounded-2xl border border-white/10 bg-black/20 p-4"><p className="text-xs uppercase tracking-[0.18em] text-white/35">房间总数</p><strong className="mt-2 block text-2xl font-black text-white">{rooms.length}</strong></div>
            <div className="rounded-2xl border border-white/10 bg-black/20 p-4"><p className="text-xs uppercase tracking-[0.18em] text-white/35">围观声望</p><strong className="mt-2 inline-flex items-center gap-2 text-lg font-black text-white"><Coins className="h-4 w-4 text-pink-300" />{user.wallet.renown}</strong></div>
            <div className="rounded-2xl border border-white/10 bg-black/20 p-4"><p className="text-xs uppercase tracking-[0.18em] text-white/35">支援记录</p><strong className="mt-2 inline-flex items-center gap-2 text-lg font-black text-white"><Flame className="h-4 w-4 text-cyan-300" />{openTickets}</strong></div>
          </div>
        </div>
      </section>
          ...
          <div className="mt-5 grid gap-5">
            <label className="block">
              ...
              <select value={selectedWorld} onChange={(event) => setSelectedWorld(event.target.value)} className="w-full rounded-[20px] border border-white/10 bg-black/20 px-4 py-3 text-sm text-white outline-none">
                {worldPacks.map((world) => <option key={world.id} value={world.id}>{world.title}</option>)}
              </select>
            </label>
            ...
            {rooms.slice(0, 5).map((room: FateRoomCard) => (
              <article key={room.id} className="rounded-[24px] border border-white/10 bg-black/20 p-4">
                <div className="flex items-start justify-between gap-4">
                  ...
                  <span className={`rounded-full border px-3 py-1 text-xs ${room.status === "recruiting" || room.isPreviewRoom ? "border-white/10 bg-white/5 text-white/68" : "border-pink-300/25 bg-pink-400/10 text-pink-100"}`}>{room.statusLabel}</span>
                </div>
      ...
      <section className="rounded-[30px] border border-white/10 bg-white/5 p-6 shadow-2xl backdrop-blur-xl">
        <div className="flex items-center gap-3"><ShieldCheck className="h-5 w-5 text-cyan-300" /><h2 className="text-2xl font-black text-white">运营说明</h2></div>
